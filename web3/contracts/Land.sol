...
contract LandToken {
    ...
    // Approve spender to spend specified amount
...
contract Land {
    ...
    constructor() public {
        landInspector = msg.sender;
        landToken = new LandToken(); // Deploy the LandToken contract
    }
    ...
    function addLand(
        ...
        // Mint tokens representing land ownership to the seller
        // Here, mint tokens equal to 1 token * 10^18 as a representation of ownership (1 token per land)
        landToken.mint(msg.sender, 1 * (10 ** uint256(landToken.decimals())));
    ...
    function payment(address payable _seller, uint _landId) public payable {
        ...
        bool success = landToken.transferFrom(msg.sender, _seller, 1 * (10 ** uint256(landToken.decimals())));
        ...
        // Transfer the payment (ether) to seller
        _seller.transfer(msg.value);

        // Emit an event that payment has been made
        emit PaymentMade(_landId, _seller, msg.sender);

        // Emit an event for successful land ownership transfer
        emit LandOwnershipTransferred(_landId, _seller, msg.sender);
    }
